...
class Customer:
    ...
    def __init__(self, customer_id, dx_queue = 0):
        self.id = customer_id
        self.state = 'entry'
        self.shopping_list = ['spices'] #+ ['fruit'] * randint(0,2) + ['drinks'] * randint(0, 2) + ['dairy'] * randint(0,2)
        self.last_state = None
        self.x = STATE_LOCATION[self.state][0][0] + dx_queue
    ...
    def next_state(self):
        ...
        # if self.state == 'checkout' and [self.x, self.y] == self.chosen_checkout.end_of_queue:
        #     self.chosen_checkout.current_customer = self
        # elif len(self.shopping_list) == 0 and self.items == 0:
        #     self.state = 'exit'
        #     self.is_checking_out = False
